...
class ListSolutionsTestCase(APITestCase):
    ...
    def test_with_non_existent_trouble(self):
        url = reverse('list-solutions', args=[self.NON_EXISTENT_TROUBLE_CODE])
        response = self.client.get(url, format='json')
        self.assertEqual(status.HTTP_404_NOT_FOUND, response.status_code)

    def test_with_existent_trouble(self):
        url = reverse('list-solutions', args=[self.EXISTENT_TROUBLE_CODE])
        response = self.client.get(url, format='json')
        trouble = Trouble.objects.get(code=self.EXISTENT_TROUBLE_CODE)
        self.assertEqual(status.HTTP_200_OK, response.status_code)
        self.assertEqual(trouble.list_solutions(), response.data)
